...
{ 
  std::cout<<"Now packing SIBs"<<std::endl;
  ...
  //std::cout<<"TAC packing"<<cell_access_related_info.tac.pack(bref)<<std::endl;
  //std::cout<<"SIB SIGN packing"<<bref.pack(sib_sign,8)<<std::endl;
  
  
  //HANDLE_CODE(bref.pack(sib_sign1,8));
  /*
  HANDLE_CODE(bref.pack(sib_sign2,8));
  HANDLE_CODE(bref.pack(sib_sign3,8));
  HANDLE_CODE(bref.pack(sib_sign4,8));
  HANDLE_CODE(bref.pack(sib_sign5,8));
  HANDLE_CODE(bref.pack(sib_sign6,8));
  HANDLE_CODE(bref.pack(sib_sign7,8));
  HANDLE_CODE(bref.pack(sib_sign8,8));
  
  
  for (int i=0;i<129;i++){
  uint8_t sign = sib_sign[i];
  HANDLE_CODE(bref.pack(sign,8));
  }
  */
  // Cybersecurity Lab: Pack the data field
  uint32_t auth_n_bytes = static_cast<uint32_t>(cell_access_related_info.cybersecurity_lab_authentication.length());
  std::vector<uint8_t> auth_val(cell_access_related_info.cybersecurity_lab_authentication.begin(), cell_access_related_info.cybersecurity_lab_authentication.end());
  uint8_t *auth_val_pointer = &auth_val[0];
  HANDLE_CODE(bref.pack_bytes(auth_val_pointer, auth_n_bytes));
...
SRSASN_CODE sib_type1_s::unpack(cbit_ref& bref)
{
  ...
  /*
      for (int i=0;i<129;i++){
    uint8_t sign = sib_sign[i];
    HANDLE_CODE(bref.unpack(sign,8));
  }
  */



  //HANDLE_CODE(bref.unpack(sib_sign1,8));


      // Cybersecurity Lab: Unpack the data field
  uint32_t auth_n_bytes = static_cast<uint32_t>(cell_access_related_info.cybersecurity_lab_authentication.length());
  std::vector<uint8_t> auth_val(cell_access_related_info.cybersecurity_lab_authentication.length());
  uint8_t *auth_val_pointer = &auth_val[0];
  HANDLE_CODE(bref.unpack_bytes(auth_val_pointer, auth_n_bytes));
  std::string temp_str(auth_val.begin(), auth_val.end());
  cell_access_related_info.cybersecurity_lab_authentication = temp_str;
  std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Digital Signature from unpacking function!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
  std::cout << cell_access_related_info.cybersecurity_lab_authentication << std::endl;
  std::cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
...
void sib_type1_s::to_json(json_writer& j) const
{
  ...
  // Cybersecurity Lab
  //j.write_int("Digital Signature", sib_sign);
