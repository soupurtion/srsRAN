...
// Sourav Definition of packing funtionl
SRSASN_CODE bit_ref::pack(uint64_t val, uint32_t n_bits)
{
  ...
  while (n_bits > 0) {
    if (ptr >= max_ptr) {
      ...
      std::cout<<"May be not packed"<<val<<std::endl;
      return SRSASN_ERROR_ENCODE_FAIL;
    }
    mask             = ((1ul << n_bits) - 1ul);
    val              = val & mask;
    uint8_t keepmask = ((uint8_t)-1) - (uint8_t)((1u << (8u - offset)) - 1u);
    //std::cout<<"From packing:"<<val<<std::endl;
...
SRSASN_CODE bit_ref::pack_sign(uint64_t  val, uint32_t n_bits)
{
  if (n_bits >= 64) {
    log_error("This method only supports packing up to 64 bits");
    return SRSASN_ERROR_ENCODE_FAIL;
...
SRSASN_CODE bit_ref::pack_bytes(const uint8_t* buf, uint32_t n_bytes)
{
  ...
  std::cout<<"Max Pointer size"<<max_ptr<<std::endl;
  if (ptr + n_bytes >= max_ptr) {
