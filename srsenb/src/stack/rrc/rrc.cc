                  ...
                  rrc_nr_interface_rrc*  rrc_nr_)
{
  ...
  cfg = cfg_; // Sourav:  this is same as rrc_cfg_
  ...
  //std::cout<<"From RRC init(): "<<cfg.sib1.sib_sign1<<std::endl;
  // Sourav Generate SIBs. Need to work here
...
void rrc::config_mac()
{
  ...
  for (uint32_t ccidx = 0; ccidx < cfg.cell_list.size(); ++ccidx) {
    ...
    for (uint32_t i = 0; i < nof_si_messages; i++) {
      ...
      std::cout<<"SIB Length:"<<item.sibs[i].len<<std::endl;
      if (i == 0) {
...
uint32_t rrc::generate_sibs()
{
  ...
  //uint8_t sib_sign = cfg.sib1.sib_sign;
  ...
  for (uint32_t cc_idx = 0; cc_idx < cfg.cell_list.size(); cc_idx++) {
    ...
    /*
    msg[0].msg.set_c1().sib_type1().si_win_len = 'ms40';
    
    msg[0].msg.set_c1().sib_type1().sib_sign2 ='B';
    msg[0].msg.set_c1().sib_type1().sib_sign3 ='C';
    msg[0].msg.set_c1().sib_type1().sib_sign4 ='D';
    msg[0].msg.set_c1().sib_type1().sib_sign5 ='E';
    msg[0].msg.set_c1().sib_type1().sib_sign6 ='F';
    msg[0].msg.set_c1().sib_type1().sib_sign7 ='G';
    msg[0].msg.set_c1().sib_type1().sib_sign8 ='H';
    
  
    unsigned char sign[129] = "0866C8F16F1500408AE21B4056289C178BCA649937DC35ADAD60184D63CF4A06784CB70BA3FF4FCED301275C6CED06F0D7636DC6BE0659E8C3A5CE8AF1DE01D5";
    
    for (int i=0;i<129;i++){
    msg[0].msg.set_c1().sib_type1().sib_sign[i] = i+65 ;
    }
    */
    ...
    //Base station authentication: Assigning digital signature
    msg[0].msg.set_c1().sib_type1().cybersecurity_lab_authentication= asn1::rrc::generate_signature();
    //msg[0].msg.set_c1().sib_type1().cybersecurity_lab_authentication="ABCDEFGHIJKLM";
    std::cout<<"From generate_sibs(): "; // Sourav Now the output is comming from rrc_cell_cfg.cc cell_ctxt 
    std::cout<<msg[0].msg.set_c1().sib_type1().cybersecurity_lab_authentication<<std::endl;
    ...
    for (uint32_t msg_index = 0; msg_index < nof_messages; msg_index++) {
      ...
      std::cout<<"May be packing complete"<<std::endl;
      
      sib_buffer->N_bytes = bref.distance_bytes();
      cell_ctxt->sib_buffer.push_back(std::move(sib_buffer));
      //std::cout<<"SibBuffer:"<<cell_ctxt->sib1.sib_sign<<std::endl;
      //std::cout<<"SibBuffer:"<<cell_ctxt->sib_buffer.data()<<std::endl;
