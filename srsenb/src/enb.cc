...
{
  // print build info
  std::cout << std::endl << get_build_string() << std::endl << std::endl; // Built in Release mode using commit d22ac0e30 on branch master.
}
...
int enb::init(const all_args_t& args_)
{
  ...
  // Sourav Validate arguments
  if (parse_args(args_, rrc_cfg, rrc_nr_cfg)) {
    srsran::console("Error processing arguments.\n");
    return SRSRAN_ERROR;
  }
  //std::cout<<"From enb::init():"<<rrc_cfg.sib1.sib_sign1<<std::endl;
  ...
  // Sourav Creating gnb layers instances
...
//Sourav LTE stack initialized from here enb_stack_lte::init()>>rrc::init()
  // Create layers
