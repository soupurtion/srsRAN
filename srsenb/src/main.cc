...
namespace bpo = boost::program_options; // boost namespace to read program options
...
void parse_args(all_args_t* args, int argc, char* argv[]) //argc will be the number of strings pointed to by argv
{
  string mcc;
  string mnc;
  string enb_id;
...
// Execute command after up and running of enb
...
int main(int argc, char* argv[])
{
  ...
  parse_args(&args, argc, argv); // for parsing argument from config file
  
  cout<<"Here is the sib configuration file: "<<args.enb_files.sib_config<<endl;
  cout<<"Here is the rrc configuration file: "<<args.enb_files.rr_config<<endl;
  cout<<"Here is the rb configuration file: "<<args.enb_files.rb_config<<endl;
  ...
  // Create eNB Sourav: Creating enB instance > calling enb.cc to perform actions
  unique_ptr<srsenb::enb> enb{new srsenb::enb(srslog::get_default_sink())};
  cout<<"SRSRAN Vlaue"<<endl;
  cout<<SRSRAN_SUCCESS<<endl;
